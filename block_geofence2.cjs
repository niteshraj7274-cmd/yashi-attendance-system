...
content = content.replace(
  /const isOutside = distance > \(Number\(centerInfo\?\.geofenceRadius\) \|\| 300\);\s*if \(isOutside && !isOfficialDuty\) \{\s*alert\(`You are Outside Centre Geofence \(\$\{Math\.round\(distance\)\}m\)\. Your attendance will be marked as 'Outside Geofence' and needs admin approval\.`\);\s*\}/g,
